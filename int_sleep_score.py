...
from tools.post_process import correct_sleep_stages
...
if __name__ == '__main__':
    ...
    parser.add_argument('--edf', type=str, default="/home/honeynaps/data/250718_CND/EDF2/CND-241111R1_M-40-OV-MI.edf")
    # parser.add_argument('--edf', type=str, default="/home/honeynaps/data/HN_DATA_MW/EDF2/SCH-241121R1_M-40-OB-MI.edf")
    ...
    sleep_label_xml_path = args.edf.replace('.edf', '_SLEEP.xml').replace('EDF2', 'EBX/SLEEP')
    sleep_labels, start_time = load_sleep_stage(sleep_label_xml_path)

    args.start_time = start_time

    arousal_preds = pred_arousal(args)
    ...
    if len(sleep_preds) > len(sleep_labels):
        sleep_preds = sleep_preds[:len(sleep_labels)]
    elif len(sleep_preds) < len(sleep_labels):
        sleep_labels = sleep_labels[:len(sleep_preds)]
                                                                                                                                                     
    print("Sleep Acc before correction:")
    ...
    corrected_sleep_stages = correct_sleep_stages(arousal_preds, 
                                                  sleep_preds, 
                                                  micro_event_preds_by_channels)

    print("Sleep Acc after correction:")
    n_corrected, n_total = 0, 0
    ...
    print("Confusion Matrix:")
    print(cm)
    print(f"EDF: {args.edf}, Event Type: {args.event_type}, TH_MUL: {args.th_mul}")
