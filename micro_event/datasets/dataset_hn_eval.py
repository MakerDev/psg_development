...
class SleepEventDatasetEBX(torch.utils.data.Dataset):
    ...
    def get_start_time(self, subject_idx: str) -> dt.datetime:
        ...
        # order = np.argsort(page_onsets)
        # page_onsets = page_onsets[order]
        start_time = page_onset_ts[0] + dt.timedelta(seconds=self.page_duration)
    ...
    def _read_events(self, xml_path: Path, channel_name: str, expand_sec=0.0) -> np.ndarray:
        ...
        marks = []
        for annot in root.iter("annotation"):
            if desired_code not in annot.findtext("description"):
                continue
    ...
    def _load_subject_channel(
        ...
        marks = self._read_events(event_xml, channel_name)
    ...
    def __init__(
        ...
        *,
        ...
        self.signals, self.marks, self.page_masks, self.user_infos, self.raw_signals = self._prepare_data()
    ...
    def __getitem__(self, idx: int):
        ...
        return feat_t, label_t, mask_t, user_info
    ...
    def _get_processed_entry(
        ...
        *,
        ...
        pages = entry["n2_pages"].astype(int) # N2 비중 카운트용 hack
        ...
        return signals.astype(np.float32), marks.astype(np.int8), page_mask.astype(np.int8), raw_signals.astype(np.float32)

    # ..................................................................

    def _prepare_data(self):
        signals, marks, page_masks, user_infos, raw_signals = [], [], [], [], []
        for entry in self.entries:
            signal, mark, page_mask, raw_signal = self._get_processed_entry(entry, pages_subset=self.pages_subset)
        ...
        return signals, marks, page_masks, user_infos, raw_signals
