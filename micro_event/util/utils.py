...
def extract_pages_mc(sequences, pages_indices, page_size, border_size=0):
    """Extracts and returns the given set of pages from multi-channel sequences
    with zero padding if border is beyond the end of the sequences.
    
    Args:
        sequences: (2-D Array) sequences of shape [N, T] where N is number of 
            channels/sequences and T is the length of each sequence.
    ...
    Returns:
        pages_data: (3-D Array) array of shape [n_pages, N, page_size+2*border_size]
    ...
    n_channels, seq_length = sequences.shape
    
    # Calculate the maximum sample index needed
    max_sample = (pages_indices.max() + 1) * page_size + border_size + 1
    
    # Extend sequences with zeros if necessary
    if max_sample > seq_length:
        extended_sequences = np.zeros((n_channels, max_sample), dtype=sequences.dtype)
        extended_sequences[:, :seq_length] = sequences
    else:
        extended_sequences = sequences
    
    # Extract pages for all channels
    pages_list = []
    for page_idx in pages_indices:
        sample_start = page_idx * page_size - border_size
        sample_end = (page_idx + 1) * page_size + border_size
        
        if sample_start < 0:
            # Handle negative start index with zero padding
            missing_samples = np.abs(sample_start)
            page_signals = extended_sequences[:, :sample_end]
            missing_part = np.zeros((n_channels, missing_samples), dtype=sequences.dtype)
            page_signals = np.concatenate([missing_part, page_signals], axis=1)
        else:
            page_signals = extended_sequences[:, sample_start:sample_end]
        
        pages_list.append(page_signals)
    
    # Stack pages: shape becomes [n_pages, n_channels, page_size+2*border_size]
