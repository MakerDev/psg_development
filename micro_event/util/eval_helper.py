...
def event_level_analysis(y_pred, y_target, y_prob=None, excel_path=None, sfreq=200, overlap_th=0.1, return_stats=False, y_sleep=None, verbose=False):
    ...
    for _, row in df_gt.iterrows():
        if row["overlap_YN"] == "Y":
            tp_events.append(row)          # TP
        else:
            fn_events.append(row)          # FN
