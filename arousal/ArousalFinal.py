...
class ArousalFinal :
    ...
    def _build_model(self, missing_channels, pretrained_dir, device):
        if self.is_ensemble:
            ...
            model_time.load_state_dict(torch.load(pretrained_path_time, map_location=device, weights_only=True))
            model_time.eval()

            model_spec = model_spec.to(device)
            model_spec.load_state_dict(torch.load(pretrained_path_spec, map_location=device, weights_only=True))
            model_spec.eval()
    ...
    def __call__(self, pretrained_dir):
        ...
        x = prep_fn(data, fs=self.fs).astype(np.float32)
        x = x.T
        x = x[start_idx:]
        total_samples = len(x)
        ...
        model, threshold, model_time, th_time, model_spec, th_spec = self._build_model(missing_channels, pretrained_dir, device)
