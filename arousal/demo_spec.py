...
def eval_fn2(model, loader, device, th=0.923):
    ...
    with torch.no_grad():
        ...
        for x, y, info, idx in loader:
            ...
            # forward
            y_pred_2d = model(x, act=True)  # (B,1,freq,T_max), sigmoid output in forward
...
def main(edf_path, save_path=None):
    ...
    arousal_dir = "/home/honeynaps/data/250718_CND/AROUS_SPEC"
    ...
    pretrained_path = "/home/honeynaps/data/saved_models_spec/NSSpecnormalW[2, 4, 6]__f1_0.8104__PAD_tech_robust_scaleno_acte_lr0.0010_fs50_ep18_auprc0.8810_th0.5659.pt"
    
    th = float(pretrained_path.split('_')[-1].replace('.pt', '').replace('th', ''))
    # th = 0.000005 # f1 약 0.4, detection ratio 95% 이상
    # th = 0.9999999 # f1 약 0.4, detection ratio 30% 정도 이지만 precision 0.9 이상
