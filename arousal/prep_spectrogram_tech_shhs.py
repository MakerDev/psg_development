...
# ==============================
def create_arousal_labels_shhs(events, total_samples, sfreq=50):
...
def process_edf_arousal_spec(edf_path, xml_path, save_dir, 
    ...
    # ------ 1) EDF 로딩 & 정규화 ------
    raw = load_edf_file(edf_path, preload=True, resample=fs, do_filter=do_filter, preset="SHHS")
    ...
    events = load_arousal_xml_shhs(xml_path)
    total_samples = x.shape[0]
    y = create_arousal_labels_shhs(events, total_samples, sfreq=fs)
    # y shape: (T,)
...
if __name__ == "__main__":
    ...
    base_dir = "/home/honeynaps/data/shhs"
    edf_dir = f"{base_dir}/edfs"
