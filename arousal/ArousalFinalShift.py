...
class ArousalFinalShift :
    ...
    def __call__(self, pretrained_dir):
        ...
        if not self.is_ensemble:
            dataset = ArousalFinalShift._DataSet('SOMNUM', x, y, transforms=transforms)
        else:
            # y에는 아무처리도 하지 않으므로, x_spec을 치팅으로 전달
            dataset = ArousalFinalShift._DataSet('SOMNUM', x, x_spec, transforms=transforms)
