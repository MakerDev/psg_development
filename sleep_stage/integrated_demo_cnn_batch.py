...
def save_to_xml(edf_path, y, save_path, base_time, fs=50, y_probs=None):
    ...
    save_sleepstage_xml(base_time, y, save_path, probs=y_probs)


class TestDataset(torch.utils.data.Dataset):
    def __init__(self, edf_path, xml_path, num_channels=9, fs=50, transforms=None, start_time=None):        
        ...
        if xml_path:
            data = load_edf_for_demo(edf_path, xml_path, fs, fill_na=False)
            ...
            X = load_only_edf(edf_path, start_time, fs)
            self.data_list.append(X)
...
def main(edf, start_time, dest):
    parser = argparse.ArgumentParser()
    parser.add_argument('--edf', type=str, default=f'/home/honeynaps/data/GOLDEN/EDF2/{edf}')
    ...
    dataset = TestDataset(args.edf, args.xml, num_channels, args.fs, transforms=transforms, start_time=args.start_time)
    ...
    pretrained_path = f'/home/honeynaps/data/shared/sleep_stage/saved_models/pretrained_asam_ver2.pt'
    ...
    with torch.no_grad():
        ...
        save_to_xml(args.edf, y_pred, save_path, base_time=args.start_time, fs=args.fs, y_probs=y_probs)
...
if __name__ == "__main__":
    csv_file = "/home/honeynaps/data/GOLDEN/EBX2/GD2_FirstEpoch.csv"
    dest = "/home/honeynaps/data/shared/sleep_stage/preds"
    ...
    with open(csv_file, 'r') as f:
        ...
        for line in lines:
            edf, start_time = line.strip().split(',')
            main(edf, start_time, dest)