...
def main(filename, missing_ch=1):
    parser = argparse.ArgumentParser()
    parser.add_argument('--edf', type=str, default=f'/home/honeynaps/data/dataset/EDF/{filename}.edf')
    parser.add_argument('--dest', type=str, default='/home/honeynaps/data/shared')
    parser.add_argument('--start_time', type=str, default='2023-07-15 22:13:00')
    ...
    pretrained_path = f'/home/honeynaps/data/shared/sleep_stage/saved_models/0.8782_MISS1_ASAM_WINDOW_WISE_Seed5_resnet18_NormaliseOnly_MissingChannel2_LR0.001_BS64_dynamic_FS50_MIXFalse_0.2_0.5_ep224.pt'
...
if __name__ == '__main__':
    filenames = """SCH-180926R2_M-70-OV-SE,
SCH-190121R2_M-10-NW-MI,
SCH-180210R2_M-40-OV-MI,
SCH-191104R1_F-60-NW-SE,
SCH-170725R1_M-10-NW-NO,
SCH-180619R2_M-20-OV-SE,
SCH-190719R2_M-30-OB-MO""".split(',')
    filenames = [f.strip() for f in filenames]

    missing_chs = [0, 1, 2]
    acc_mean = [0] * len(missing_chs)
    for filename in filenames:
        accs = [0] * len(missing_chs)
        for missing_ch in missing_chs:
            accs[missing_ch] = main(filename, missing_ch=missing_ch)
            acc_mean[missing_ch] += accs[missing_ch]
        
        print(f"{filename}:", end="")
        for missing_ch in missing_chs:
            print(f" {missing_ch}: {accs[missing_ch]:.4f}", end="")
        print()

    acc_mean = [a / len(filenames) for a in acc_mean]
    print("Mean Accuracy:", end="")
    for missing_ch in missing_chs:
        print(f" {missing_ch}: {acc_mean[missing_ch]:.4f}", end="")
    print()