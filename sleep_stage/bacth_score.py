...
if __name__ == '__main__':
    ...
    parser.add_argument('--edf', type=str, default='/home/honeynaps/data/dataset/EDF/SCH-190924R2_F-50-NW-MI.edf')
    ...
    FEPs = {}
    if args.start_time :
        with open(args.start_time, 'r') as tr :
            lines = tr.readlines()
            for line in lines :
                line = line.strip()
                toks = line.split(',')
    ...
    edfList = [edf for edf in os.listdir(args.edf) if edf.endswith('.edf')]
    for i in range(len(edfList)) :
        edfName = edfList[i]
        edfFile = args.edf + os.sep + edfName
        if len(FEPs) > 0 and not edfName in FEPs :
            print(f'{edfName} : Recording time not found!!!')
            continue
        #--IF
        start_time = FEPs[edfName]
        ...
        dataset = TestDataset(edfFile, args.xml, num_channels, args.fs, transforms=transforms, start_time=start_time)
        ...
        with torch.no_grad():
            ...
            edf_name  = edfName  #os.path.basename(args.edf)
            save_path = os.path.join(save_dir, edf_name.replace('.edf', '_SLEEP.xml'))
            save_to_xml(edfFile, y_pred, save_path, base_time=start_time, fs=args.fs)
